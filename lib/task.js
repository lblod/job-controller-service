...
import { TASK_TYPE, PREFIXES, TASK_URI_PREFIX, STATUS_BUSY,STATUS_SCHEDULED,STATUS_SUCCESS } from '../constants';
...
export async function isTaskExist(graph, job, index, operation) {
    const queryStr = `
        ${PREFIXES}
        SELECT ?task {
            GRAPH <${graph}> {
