...
export async function getBatchTasksToConsiderForScheduling(){
  const result = await query(`
    PREFIX task: <http://redpencil.data.gift/vocabularies/tasks/>
    PREFIX adms: <http://www.w3.org/ns/adms#>
    PREFIX ext: <http://mu.semte.ch/vocabularies/ext/>
    
    SELECT DISTINCT ?current ?status WHERE {
      VALUES ?status {
        <http://redpencil.data.gift/id/concept/JobStatus/success>
        <http://redpencil.data.gift/id/concept/JobStatus/failed>
      }
      ?current a task:Task .
      ?current adms:status ?status .
      FILTER NOT EXISTS {
        ?next <http://vocab.deri.ie/cogs#dependsOn> ?current.
      }
      FILTER NOT EXISTS {
        ?current ext:checkedForScheduling ?date
      }
    } limit ${BATCH_SIZE}`)
  return result.results.bindings.map(binding => {
    return {
      uri: binding.current.value,
      status: binding.status.value,
    };
  });
}

export async function markTaskScheduled(taskUri){
  await update(`
    PREFIX ext: <http://mu.semte.ch/vocabularies/ext/>
    PREFIX task: <http://redpencil.data.gift/vocabularies/tasks/>
    INSERT {
      GRAPH ?g {
        ?task ext:checkedForScheduling ?now .
      }
    } WHERE {
      GRAPH ?g {
        VALUES ?task {
